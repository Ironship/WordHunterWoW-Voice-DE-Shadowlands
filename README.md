# QuestWordHunter — German Voiceover: Shadowlands

The German a quest giver says out loud, for Shadowlands: quest ids
**56,055–64,000**.

**7 clips, 1 minute.** Both are counted from this pack's own duration table in
...
`sounds/` is gitignored, and `.gitignore` says why: it is under a megabyte here
so far and will be around seven gigabytes across the twelve packs, and how that
should ship has not been decided. So a checkout of this repository is not
installable by itself — it is the manifest, the licence and the duration table,
and no sound.
...
```
python Tools/build_pack.py --only Shadowlands --out "…/Interface/AddOns"
```
